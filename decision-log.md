...
**2026-09-16** — Success target left unchanged at MASE ≤ 0.85 at week 4; the one permitted revision is used and closed. Data since August shows year-to-year level drift (27% spread) exceeding seasonal amplitude (23%), and seasonal-naive cannot track level — it copies last year's level wholesale — which suggests 0.85 may be a softer bar than assumed when it was set on 2026-08-26. But that is a mechanism, not a measurement: no model has been fit, so how much of that error a model actually recovers is unknown. Revising a pre-committed target on an untested hypothesis costs more than it gains. Target is fixed. No further revision, including after model scores. Charter §2.6 revision clause closed.
**2026-09-17** — Fold origins are now pinned by date. Reason being the series is a live one and I wanted to have good comparibility in the model. There are a total of 12 folds. The test windows are 6 weeks long. The dates between each fold is about 4 weeks. Each fold trains on everything up to its origin and forecasts the 6 weeks after it then we use that number to try to compare the answer to the answer key which is 6 weeks later. The dates that I ended up choosing where starting at the first origin 2025-03-23 and the last origin being 2026-01-25 and the fold tests end on 2026-03-08. The last 26 weeks of my actual date are reserved for when I want to do a final test of my model. The model will have spare weeks. Fold 12 test ends 2026-03-08. used_set ends 2026-03-15. One week unused this is because we have 845 rows, 12 folds stepped 4, 6-week tests and they don't divide evenly, so one week is left over and since origins are pinned, that gap grows every pull. used_set gets longer, folds stay put. By December it's a couple months of weeks nothing trains on. That's the accepted cost of comparability.
